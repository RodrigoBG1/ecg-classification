...
- Loads pre-computed `{model}_metrics.json` and prediction arrays from `results/metrics/`
- Generates:
  - **Grouped bar chart**: AUC-ROC, F1, AUPRC, accuracy across 3 models
  - **Overlaid macro ROC curves**: TPR vs FPR for all models on a single axes
  - **Radar/spider chart**: 5-axis plot of AUC, F1, AUPRC, sensitivity, specificity
  - **AUC heatmap**: 3×5 matrix (models × classes)
  - **F1 heatmap**: 3×5 matrix (models × classes)
  - **Bootstrap AUC comparison**: 1000-sample bootstrap with p-values and 95% CI
- Saves comparison figures to `results/figures/comparison_*.{pdf,png}`
...
Comparison:
- `comparison_grouped_bars.{pdf,png}` — AUC, F1, AUPRC, accuracy grouped by model
- `comparison_macro_roc.{pdf,png}` — macro ROC overlaid for all models
