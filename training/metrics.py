...
def compute_metrics(
    ...
    Returns:
        Dictionary with keys:
            ...
            and per-class f1_<CLASS>, precision_<CLASS>, recall_<CLASS>.
    """
    ...
    metrics: dict[str, float] = {
        ...
        "precision_macro": float(
            precision_score(y_true, y_pred, average="macro", **shared_kwargs)
        ),
        "recall_macro": float(
            recall_score(y_true, y_pred, average="macro", **shared_kwargs)
        ),
        "f1_macro": float(
            f1_score(y_true, y_pred, average="macro", **shared_kwargs)
        ),
    }
...
if __name__ == "__main__":
    ...
    y_true = rng.integers(0, 2, size=(32, 5)).astype(float)
    y_logits = rng.standard_normal((32, 5))
    m = compute_metrics(y_true, y_logits)
    print(format_metrics(m))
    for cls in CLASSES:
        print(f"  {cls}: F1={m[f'f1_{cls}']:.3f}  P={m[f'precision_{cls}']:.3f}  R={m[f'recall_{cls}']:.3f}")
